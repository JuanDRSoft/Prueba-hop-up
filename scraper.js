...
async function checkReplicas() {
    ...
    const categorias = ['Ametralladora', 'Pistola', 'Fusil / Subfusil', 'Rifle', 'Escopeta'];
    ...
    if (algunaCategoriaVacia) {
        ...
        console.log('No se envía Email: Todas las categorías en todas las tiendas tienen réplicas.');
    }
}
...
async function sendEmail(tiendasConCategoriasVacias) {
    ...
    for (let tienda in tiendasConCategoriasVacias) {
        textoEmail += `La tienda ${tienda} no tiene réplicas en las siguientes categorías: ${tiendasConCategoriasVacias[tienda].join(', ')}. \n`;
    }
...
// MAD TACTICAL
    ...
    async function scrapeMadTactical(url, tiendaNombre, categoria, estado) {
        ...
        for (let i = 0; i < marcaCheckboxLength; i++) {
            ...
            const marcaTienda = await page.$eval(`#ul_layered_manufacturer_0 li:nth-of-type(${i + 1}) label a`, a => a.textContent.trim().replace(/\s*\(\d+\)$/, ''));
            const marcaDoc = await marcasCollection.findOne({ [`tienda_nombres.${tiendaNombre}`]: marcaTienda });
            const marca = marcaDoc ? marcaDoc.nombre : marcaTienda;
    
            let hasNextPage = true;
            while(hasNextPage) {
                let productos = await page.$$eval('.ajax_block_product', (productos, categoria) => productos.map(producto => {
                    const nombre = producto.querySelector('h5 a').textContent.trim();
                    const precio = parseFloat(producto.querySelector('.price').textContent.trim().replace('€', '').replace(',', '.'));
                    const enlace = producto.querySelector('.product_img_link').getAttribute('href');
                    const descripcion = producto.querySelector('.product-desc').textContent.trim();
        ...
        const replicasToDelete = await replicasCollection.find({ 
            categoria: categoria, 
            tienda: tiendaNombre, 
            _id: { $nin: newProductIds } 
        }).toArray();
        ...
        const replicaNamesToDelete = replicasToDelete.map(replica => replica.nombre);
        console.log(`----------Réplicas eliminadas: ${replicaNamesToDelete.join(', ')}`);
        ...
        console.log(`----------Los productos antiguos de la categoría "${categoria}" y de la tienda "${tiendaNombre}" se han limpiado.`);
...
// HOBBY EXPERT
    ...
    async function scrapeHobbyExpertFusilesSubfusiles(urls, tiendaNombre, estado) {
        ...
        for (let url of urls) {
            await page.goto(url);
            let tipos = await page.$$eval('div.lvl3-category-banner', divs => divs.map(div => ({
            ...
            for(let tipo of tipos) {
                ...
                while (hasNextPage) {
                    ...
                    let productCards = await page.$$eval('.product-item-info', cards => cards.map(card => ({
                        url: card.querySelector('a.product.photo.product-item-photo').href,
                        outOfStock: card.querySelector('.stock span')?.textContent.trim() === "No está disponible"
                    })));
                    ...
                    for(let productCard of productCards) {
                        ...
                        const nombre = await currentPage.$eval('h1.page-title .base', node => node.textContent.trim());
                        const precio = parseFloat(await currentPage.$eval('.product-info-price .price', node => node.textContent.trim().replace('€', '').replace(',', '.')));
                        let marcaTienda;
                        try {
                            marcaTienda = await currentPage.$eval('.amshopby-option-link a', node => node.getAttribute('title'));
                        } catch (error) {
                        ...
                        let descripcionElements = await currentPage.$$('.data .description .value p');
                        let descripcion = '';
                        for(let el of descripcionElements){
                            try {
                                descripcion += await currentPage.evaluate(el => el.textContent, el);
                            } catch (error) {
                        ...
                        for (let i = 0; i < 2; i++) {
                            try {
                                await currentPage.waitForSelector('.fotorama__stage .fotorama__stage__shaft .fotorama__active img.fotorama__img', { timeout: 5000 });
                                imagen = await currentPage.$eval('.fotorama__stage .fotorama__stage__shaft .fotorama__active img.fotorama__img', node => node.getAttribute('src'));
                                if (imagen) {
                                    break;
                                }
                            } catch (error) {
                                console.log(`Intento ${i+1} para obtener la imagen falló, reintentando... Error: ${error}`);
                                if (i === 1) {
                                    try {
                                        imagen = await currentPage.$eval('img.fotorama__img', node => node.getAttribute('src'));
                                    } catch (altError) {
                                        console.error(`También falló al obtener la imagen alternativa. Error: ${altError}`);
                                    }
                                }
                            }
                        }
                        ...
                        if (!existingProducto) {
                            ...
                            await replicasCollection.updateOne({ _id: existingProducto._id }, { $set: { 
                                precio: precio, 
                                descripcion: descripcion, 
                                imagen: imagen, 
                                estado: estado 
                    ...
                    try {
                        const loadMoreButton = await page.waitForSelector('button.primary.amscroll-load-button', { timeout: 3000, visible: true });
        ...
        const productsToDelete = await replicasCollection.find({ 
            categoria: "Fusil / Subfusil", 
            tienda: tiendaNombre, 
        ...
        console.log(`----------Los productos antiguos de la categoría "Fusil / Subfusil" y de la tienda "${tiendaNombre}" se han limpiado.`);
...
module.exports = { checkReplicas, borrarReplicas, scrapeMadTactical, scrapeHobbyExpertFusilesSubfusiles, scrapeHobbyExpert };
