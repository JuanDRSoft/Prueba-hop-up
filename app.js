...
const { checkReplicas, borrarReplicas, scrapeMadTactical, scrapeHobbyExpertFusilesSubfusiles, scrapeHobbyExpert } = require('./scraper');
...
app.post('/contacto', async (req, res) => {
  ...
  try {
    ...
    res.render('contacto', { titulo: '¡MENSAJE ENVIADO CORRECTAMENTE!', mensaje: 'Tu consulta es importante para nosotros y nos aseguraremos de responderla lo antes posible.' });
} catch (error) {
    console.error(`Error al enviar el correo: ${error}`);
    res.render('contacto', { titulo: '¡NO SE HA ENVIADO EL MENSAJE!', mensaje: `Ha ocurrido un error al enviar tu mensaje. Por favor inténtalo nuevamente. ${error.message}` });
}
...
app.listen(port, async () => {
  ...
  console.log('<<<<<<<<<<<<<<<Scraping Inicial "Fusil/Subfusil" de la tienda "Mad Tactical"...');
  await scrapeMadTactical('https://madtactical.es/6-fusiles-y-subfusiles', "Mad Tactical", "Fusil / Subfusil", "Nuevo");
  console.log('>>>>>>>>>>>>>>>¡Hecho!');

  console.log('<<<<<<<<<<<<<<<Scraping Inicial "Fusil/Subfusil" de la tienda "Hobby Expert"...');
  ...
  await scrapeMadTactical('https://madtactical.es/83-sniper-rifle-francotirador', "Mad Tactical", "Rifle", "Nuevo");
  console.log('>>>>>>>>>>>>>>>¡Hecho!');

  console.log('<<<<<<<<<<<<<<<Scraping Inicial "Rifle" from "Hobby Expert"...');
  await scrapeHobbyExpert('https://www.hobbyexpert.es/replicas-de-airsoft/rifles-de-francotirador-airsoft', "Hobby Expert", "Rifle", "Nuevo");
  console.log('>>>>>>>>>>>>>>>¡Hecho!');

  //Categoría "Ametralladora"
  console.log('<<<<<<<<<<<<<<<Scraping Inicial "Ametralladora" from "Mad Tactical"...');
  await scrapeMadTactical('https://madtactical.es/85-ametralladoras', "Mad Tactical", "Ametralladora", "Nuevo");
  console.log('>>>>>>>>>>>>>>>¡Hecho!');

  console.log('<<<<<<<<<<<<<<<Scraping Inicial "Ametralladora" from "Hobby Expert"...');
  await scrapeHobbyExpert('https://www.hobbyexpert.es/replicas-de-airsoft/apoyo-airsoft', "Hobby Expert", "Ametralladora", "Nuevo");
  console.log('>>>>>>>>>>>>>>>¡Hecho!');
...
// ESCANEO PERIÓDICO
const taskEscaneoPeriodico = cron.schedule('0 * * * *', async () => {
  // Categoría "Fusil / Subfusil"
  console.log('<<<<<<<<<<<<<<<Scraping Inicial "Fusil/Subfusil" de la tienda "Mad Tactical"...');
  await scrapeMadTactical('https://madtactical.es/6-fusiles-y-subfusiles', "Mad Tactical", "Fusil / Subfusil", "Nuevo");
  console.log('>>>>>>>>>>>>>>>¡Hecho!');
